...
class ODITSTrainer:
    ...
    def train_step(self, episodes_data, max_ep_len):
        ...
        total_loss = 0.0
        # Initialize random number between 0 and max_ep_len-1
        random_ts = torch.randint(1, max_ep_len-21, (1,)).item()
        for ts in range(random_ts, random_ts+20):
            h = h_0
            # 提取所有批次样本的该时间步的数据    
            ...
            target_utility = reward + self.gamma * next_max_utility     
            # 计算重构损失 - 使用MSE损失
            Q_loss = torch.nn.functional.mse_loss(current_utility, target_utility.detach())
