...
from Trainer import SequenceTrainer_lbf_wo_goal, BaseTrainer
from TestGame import Test
from Agent.Adhoc_DT import Adhoc_DT_wo_goal
            ...
            for batch_idx, episodes_data in enumerate(pbar):
                ...
                if batch_idx % 10 == 0:
                    logger.info(f"Epoch [{epoch+1}/{num_epochs}], Batch [{batch_idx}/{len(train_loader)}]\n "
                                ...
                                f"MSE Loss R: {loss_dt['mse_loss_r']:.4f}")
...
if __name__ == "__main__":
    ...
    env = "LBF"
    config = load_config(f"./config/{env}_config_wo_goal.yaml")
    ...
    # 初始化 Trainer
    trainer = SequenceTrainer_lbf_wo_goal(
        model=dt,
